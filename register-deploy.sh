...
do
  ...
  then
    APP="${BASH_REMATCH[1]}"
    curl -s -X POST "$WERCKER_KUBECTL_INFLUXDB_DSN&precision=s" --data-binary "deploy,app=$APP,revision=$REVISION,author_name=$AUTHOR_NAME,author_email=$AUTHOR_EMAIL,msg=$COMMIT_SUBJECT value=1.0 $TS"
  fi 
done < "$LOG"
